...
import com.nikola.githubevents.domain.model.PullRequestEventPayload
...
data class PullRequestEventDto(
    ...
    data class PullRequestEventPayloadDto(
        val action: String,
        val number: Int,
        @Json(name = "distinct_size")val pullRequest: PullRequestDto
    )
    ...
    override fun toDomain(): GitHubEvent = GitHubEvent(
        id,
        type,
        actor.toDomain(),
        repo.toDomain(),
        PullRequestEventPayload(
            payload.action,
            payload.number,
            PullRequestEventPayload.PullRequest(
